...
# Util Functions
def conv(in_planes, out_planes, kernel_size=3, stride=1, padding=1, activefun=nn.LeakyReLU(0.1, inplace=True)):
...
def deconv(in_planes, out_planes, kernel_size=4, stride=2, activefun=nn.LeakyReLU(0.1, inplace=True)):
    ...
    return nn.Sequential(
        nn.ConvTranspose2d(
            in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=1, bias=True,), #反卷积
        activefun,
    )
...
class SegNet2D(nn.Module):
    def __init__(self):
        ...
        self.conv3 = conv(cps[2], cps[3], kernel_size=3, stride=2, activefun=self.activefun) #为了对齐增加了padding
    ...
    def forward(self, x):
        ...
        out_conv0 = x
        out_conv1 = self.conv1_1(self.conv1(out_conv0)) #[1,128,90,192] 连续的情况下是[65,128,96,192]
        out_conv2 = self.conv2_1(self.conv2(out_conv1)) #[1,256,45,96]
        out_conv3 = self.conv3_1(self.conv3(out_conv2)) #[1,512,24,48]
        out_conv4 = self.conv4_1(self.conv4(out_conv3)) #[1,512,12,24]
        out_conv5 = self.conv5_1(self.conv5(out_conv4)) #[1,512,6,12]

        out_deconv5 = self.deconv5(out_conv5) #[1,512,6,12]
        out_deconv5_1 = self.deconv5_1(torch.cat((out_conv4, out_deconv5), 1)) #[1,512,12,24]

        out_deconv4 = self.deconv4(out_deconv5_1) #[1,512,24,48]
        out_deconv4_1 = self.deconv4_1(torch.cat((out_conv3, out_deconv4), 1)) #[1,512,24,48]

        out_deconv3 = self.deconv3(out_deconv4_1) #为了对齐做了切片
        ...
        raw_seg = self.last_conv(out_deconv1_1) #[1,192,384]
