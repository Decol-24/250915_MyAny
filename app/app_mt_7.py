...
import random
import pickle
from app_utils import app_no_dump,preprocess_fn,creat_folder,time_counter
...
def encode(data_id,inter_out,final_out_dir,once_img_num,time_c,threshould):
    # 根据保存的结果计算cos sim，并压缩
    
    if once_img_num > len(inter_out):
        once_img_num = len(inter_out)
    
    out_compress = [None] * once_img_num
    out_replaced_idx = [None] * once_img_num

    time_c.start()
    for i in range(once_img_num):
        cos_sim = cos_5(inter_out[i][0])
        out_compress[i],out_replaced_idx[i] = encode_5(inter_out[i][0], cos_sim, threshould)
...
def main():
    ...
    once_img_num = 10
    run_times = 10
    for data_id in range(0,run_times):
        if args.mode == 0:
            time_c_1 = time_counter()
            time_c_2 = time_counter()
            app_no_dump(args.image_dir,inter_out_dir,data_id,args.xmodel,once_img_num,time_c_1)

            encode(data_id,inter_out_dir,final_out_dir,once_img_num,time_c_1,time_c_2,threshould)
