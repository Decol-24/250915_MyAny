...
def cos_9(np_input,threshould):
    ...
    for idx in range(196):
        ...
        cos_sim = np.zeros(196)
        for idx_2 in range(idx+1,196):
        
            cos_sim[idx_2] = np.dot(outptut_norm[idx],outptut_norm[idx_2]) #idx的token和后面所有token的相关性
        
        cos_sim_idx = cos_sim > threshould

        np.logical_or(replaced,cos_sim_idx,out=replaced)
...
def encode(data_id,inter_out_dir,final_out_dir,runTotal,time_c,threshould):
    ...
    for i in range(runTotal):
        out_compress[i],out_replaced_idx[i] = cos_9(inter_out[i][0],threshould)
        if i % 100 == 0:
...
def main(args):
    ...
    for data_id in range(0,2):
        if args.mode == 0:
            ...
            encode(data_id,inter_out_dir,final_out_dir,runTotal,time_c,threshould)
        with open('log.txt','a') as f:
...
def main_loop(args):
    ...
    final_out_dir = './final_out_c9_img'
    creat_folder(final_out_dir)
    ...
    for threshould in range(80,95): #!
        ...
        for data_id in range(10,13): #!
            ...
            if args.mode == 1:
                encode(data_id,inter_out_dir,path,runTotal,time_c,_threshould)

            with open('log.txt','a') as f:
                print('Now cos_9 avg_time:',file=f)
                print('{}'.format(time_c.avg_time()),file=f)
