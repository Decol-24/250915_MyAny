...
# 把每个视差的预测结果乘上一个自然数数列然后求和降为标量，这样根据数值就能判断视差结果
def disparity_regression(x, maxdisp): # x[1,48,64,128]
    assert len(x.shape) == 4
    disp_values = torch.arange(0, maxdisp, dtype=x.dtype, device=x.device)
    disp_values = disp_values.view(1, maxdisp, 1, 1) #[1,48,1,1]
...
def groupwise_correlation(fea1, fea2, num_groups):
    ...
    # fea1 和 fea2 按元素相乘，然后分成多个组，在每个组内求平均
...
def build_gwc_volume(refimg_fea, targetimg_fea, maxdisp, num_groups):
    ...
    for i in range(maxdisp):
        if i > 0:
            ...
            #volume的第2维是固定为i，其他四个维度被等号右侧的tensor依次填充
        else:
            volume[:, :, i, :, :] = groupwise_correlation(refimg_fea, targetimg_fea, num_groups)
    volume = volume.contiguous()
    # volume[B, num_groups, maxdisp, H, W] 按不同的视差创造相关体，在视差d下左图w轴的[d:]和右图w轴的[:-d]相乘得到代价体，存到volume的[d:]中
    return volume
