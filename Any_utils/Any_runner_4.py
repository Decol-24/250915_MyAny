...
class my_runner(object):
    ...
    def train(self, train_loader, val_loader, scheduler, optimizer):
        ...
        criterion_1 = loss_sparse(self.s.start_disp, self.s.end_disp, self.logger, dis_arange[0], self.s.sparse)
        ...
        for ep in range(1, self.s.train_EPOCHS + 1):
            ...
            self.train_one_epoch_amp(ep,train_loader,optimizer,criterion,criterion_1,criterion_2,criterion_3,self.s.device)
    ...
    def train_one_epoch_amp(self,ep,train_loader,optimizer,criterion,criterion_1,criterion_2,criterion_3,device):
        ...
        for batch_idx, (imgL, imgR, disp_true) in enumerate(train_loader):
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                preds, disp_up = self.model(imgL, imgR)
                loss = criterion([disp_up], disp_true)[0]
                loss_1 = criterion_1(preds[0], disp_true)
            ...
            train_loss_3 += (loss_3.item()) / valid_sample
            ...
            if batch_idx % 200 == 0:
                self.logger.info("step: [{}/{}] | loss_1: {:.3f} | loss_2: {:.3f} | loss_3: {:.3f} | loss: {:.3f} | epe: {:.3f}"
                                 .format(batch_idx+1, len(train_loader), train_loss_1 / (batch_idx+1), train_loss_2 / (batch_idx+1), train_loss_3 / (batch_idx+1), train_loss / (batch_idx+1), train_epe / (batch_idx+1)))
